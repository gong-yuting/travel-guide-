...
const Globe3D = {
  ...
  _drawContinentPoly(x, poly) {
    ...
    cg.addColorStop(0, 'rgba(255,240,245,0.35)');
    cg.addColorStop(0.5, 'rgba(255,220,235,0.28)');
    cg.addColorStop(1, 'rgba(240,200,220,0.22)');
    x.fillStyle = cg;
    x.fill();
  ...
  _draw(t) {
    ...
    // Deep space bg
    const bg = x.createRadialGradient(cx, cy, r * 0.1, cx, cy, r * 2);
    bg.addColorStop(0, 'rgba(30,20,35,0.3)'); bg.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    this._stars.forEach(s => {
      ...
      x.fillStyle = 'rgba(220,200,240,' + a + ')';
      x.beginPath(); x.arc(sx, sy, s.r, 0, Math.PI * 2); x.fill();
      // Occasionally add cross sparkle
      if (a > 0.45) {
        x.strokeStyle = 'rgba(255,240,255,' + (a - 0.45) * 2 + ')';
        x.lineWidth = 0.3;
    ...
    // Atmosphere rings
    for (let i = 0; i < 2; i++) {
      const ag = x.createRadialGradient(cx, cy, r * (0.88 + i * 0.04), cx, cy, r * (1.15 + i * 0.08));
      ag.addColorStop(0, 'rgba(255,200,220,0)');
      ag.addColorStop(0.4, 'rgba(255,190,210,' + (0.04 + i * 0.02) + ')');
      ag.addColorStop(0.7, 'rgba(255,170,195,' + (0.03 + i * 0.01) + ')');
      ag.addColorStop(1, 'rgba(0,0,0,0)');
      x.fillStyle = ag; x.beginPath(); x.arc(cx, cy, r * (1.15 + i * 0.08), 0, Math.PI * 2); x.fill();
    }

    // Ocean sphere with specular highlight
    const sg = x.createRadialGradient(cx - r * 0.22, cy - r * 0.28, r * 0.05, cx, cy, r);
    sg.addColorStop(0, 'rgba(240,230,245,0.75)');
    sg.addColorStop(0.15, 'rgba(215,200,225,0.9)');
    sg.addColorStop(0.5, 'rgba(180,165,195,0.95)');
    sg.addColorStop(0.85, 'rgba(155,140,170,0.98)');
    sg.addColorStop(1, 'rgba(130,115,150,1)');
    x.fillStyle = sg;
    ...
    // Islands
    x.fillStyle = 'rgba(255,245,250,0.2)';
    ...
    // Grid
    x.strokeStyle = 'rgba(255,255,255,0.06)'; x.lineWidth = 0.35;
    ...
    for (let i = 0; i < this._dots.length; i++) {
      ...
      if (hov) {
        ...
        pr.addColorStop(0, 'rgba(255,150,180,0.3)');
      ...
      gg.addColorStop(0, 'rgba(255,240,250,' + (hov ? 1 : 0.65) + ')');
      gg.addColorStop(0.4, 'rgba(255,160,190,' + (hov ? 0.6 : 0.25) + ')');
      gg.addColorStop(1, 'rgba(0,0,0,0)');
      x.fillStyle = gg; x.beginPath(); x.arc(p2.x, p2.y, gr, 0, Math.PI * 2); x.fill();

      // Center
      x.fillStyle = hov ? '#fff' : '#ff7099';
      ...
      if (hov) {
        ...
        const rh = 24, rx = 8;
        x.fillStyle = 'rgba(255,248,252,0.96)'; x.strokeStyle = 'rgba(255,150,180,0.7)'; x.lineWidth = 1.2;
        ...
        x.fillStyle = '#4a3040'; x.textAlign = 'left';
        x.fillText(txt, tx + 8, ty + 18); x.textAlign = 'start';
      }
    }

    // Rim light
    x.strokeStyle = 'rgba(255,210,225,0.15)'; x.lineWidth = 2;
